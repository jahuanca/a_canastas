...
export class FormSimpleComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if(this.data!=null && this.isVisible){
      ...
      if(this.editando){
        for (let j = 0; j < this.itemsFormulario.length; j++) {
          const element = this.itemsFormulario[j];
          if(element.type=='range-time-picker'){
            for (let i = 0; i < element.value.length; i++) {
              const elementI = element.value[i];
              this.itemsFormulario[j].data[i]=new Date(this.data[elementI]);
            }
          }
        }
      }
